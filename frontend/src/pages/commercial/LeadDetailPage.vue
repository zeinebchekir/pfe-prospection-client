<template>
    ...
    <div class="flex-1 flex flex-col min-w-0">
      ...
      <header class="h-14 sm:h-16 border-b border-border bg-white sticky top-0 z-40 px-3 sm:px-6 flex items-center justify-between shadow-sm">
        ...
        <div class="flex items-center gap-1.5 sm:gap-2 flex-shrink-0">
          ...
          <Button variant="outline" @click="showRapportsDrawer = true" class="gap-2">
            <BarChart2 class="w-4 h-4" />
            Rapports LinkedIn
            <Badge v-if="rapportsCount > 0" class="ml-1 bg-blue-100 text-blue-700 border-0 text-xs">
              {{ rapportsCount }}
            </Badge>
          </Button>
        </div>
      ...
      <main v-else class="flex-1 p-6 md:p-8 overflow-y-auto">
        <div class="max-w-[1100px] mx-auto space-y-6">
          ...
          <div class="flex justify-end gap-3 pb-8">
            ...
            <Button 
              v-else
              @click="showAnalysisConfirm = true"
  ...
  />
  <LeadRapportsDrawer
    :is-open="showRapportsDrawer"
    :lead-id="displayLead?.id"
    :lead-nom="displayLead?.nom"
    :lead="displayLead"
    @update:is-open="showRapportsDrawer = $event"
    @nouveau-rapport="showRapportsDrawer = false; showAnalysis = true"
  />

  <!-- ⚠️ Modal de confirmation analyse -->
  <Dialog :open="showAnalysisConfirm" @update:open="showAnalysisConfirm = $event">
    <DialogContent class="max-w-md">
      <DialogHeader>
        <DialogTitle class="flex items-center gap-2 text-amber-700">
          <AlertTriangle class="w-5 h-5 text-amber-500" />
          Lancer une analyse LinkedIn ?
        </DialogTitle>
        <DialogDescription class="pt-2 space-y-2">
          <p class="text-sm text-foreground">
            Cette action va consommer des crédits API (LinkedIn, Serper, IA) pour analyser le potentiel de ce lead.
          </p>
          <div v-if="lastRapportDate" class="flex items-center gap-2 p-3 rounded-lg bg-amber-50 border border-amber-200 text-amber-800 text-sm">
            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>
            <span>Dernier rapport généré le <strong>{{ lastRapportDate }}</strong></span>
          </div>
          <p v-else class="text-sm text-muted-foreground italic">
            Aucun rapport précédent pour ce lead.
          </p>
          <p class="text-sm text-muted-foreground pt-1">Confirmez-vous le lancement de l'analyse ?</p>
        </DialogDescription>
      </DialogHeader>
      <DialogFooter class="gap-2 pt-2">
        <Button variant="outline" @click="showAnalysisConfirm = false">Annuler</Button>
        <Button @click="showAnalysisConfirm = false; showAnalysis = true" class="bg-tacir-blue hover:opacity-90 text-white gap-2">
          <Sparkles class="w-4 h-4" />
          Confirmer et lancer
        </Button>
      </DialogFooter>
...
import LeadRapportsDrawer from '@/components/leads/LeadRapportDRawer.vue'
...
const showRapportsDrawer = ref(false)
const rapportsCount = ref(0)
const lastRapportDate = ref('')
...
onMounted(async () => {
  ...
  try {
    const { data } = await axios.get(`/potential-linkedin/entreprise/${displayLead.value?.id}/all`)
    rapportsCount.value = data.length
    if (data.length > 0 && data[0].created_at) {
      lastRapportDate.value = new Date(data[0].created_at).toLocaleDateString('fr-FR', {
